<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="sebrand.aspx.cs" Inherits="wxtest.UILayer.MetalPlate.sebrand" %>
...
<body>
    ...
    <div data-role="page" class="ui-page-theme-a">
        ...
        <div role="main" class="ui-content">
            <div id="pagecontent">
                <%if (CarInfoView.BrandList != null && CarInfoView.BrandList.Count() > 0)
                    {%>
                <ul data-role="listview" data-filter="true" data-inset="true">
                    <%string inistr = ""; %>
                    <%foreach (var item in CarInfoView.BrandList)
                        {
                            if (inistr != item.Initial)
                            {
                                inistr = item.Initial;
                    %>
                    <li style="background-color: gray">
                        <div style="text-align:center"><strong><%=inistr %></strong></div>
                    </li>
                    <%} %>
                    <li><a href="semodel.aspx?brand=<%=item.Id.ToString() %>" rel="external"><%=item.Name.ToString() %></a></li>
                    <%} %>
                </ul>
                <%} %>
            </div>
